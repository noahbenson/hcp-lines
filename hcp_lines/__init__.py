...
from neuropythy.hcp           import subject as hcp_subject
from neuropythy.util          import (curry, pseudo_path, address_data, config)
from neuropythy.datasets.core import (Dataset, add_dataset)
from neuropythy.datasets.hcp  import HCPMetaDataset
...
class HCPLinesDataset(HCPMetaDataset):
    ...
    '''
    def __init__(self,
                 custom_directory=None, create_mode=0o775, create_directories=True, meta_data=None,
                 metadata_path=None, genetic_path=None, behavioral_path=None,
                 trust_exclusions=True):
        # find the lines-path:
        line_path = config['hcp_lines_path']
        if line_path is None and config['hcp_subject_paths'] is not None:
            for path in config['hcp_subject_paths']:
                for pp in [('retinotopy', 'lines'), ('lines',)]:
                    pp = os.path.join(path, *pp)
                    if os.path.isdir(pp): line_path = pp
                if line_path is not None: break
        if line_path is None:
            from getpass import getuser
            line_path = os.path.join('/','acadia',getuser(),'Projects','HCP','retinotopy','lines')
        if line_path is None or not os.path.isdir(line_path):
            from getpass import getuser
            line_path = os.path.join('/','Volumes','server','Projects','HCP','retinotopy','lines')
        if line_path is None or not os.path.isdir(line_path):
            raise ValueError('Could not find HCP lines path!')
        # one day: if we get here without a custom lines-path, we'll arrange to download from the
        # OSF page for this project.
        self.source_path = line_path
    ...
    def pseudo_path(source_path, cache_directory):
        ...
        HCP-lines raw data.
        '''
        return pseudo_path(source_path, cache_path=cache_directory)
    ...
    def _calculate_subject_distances(paths, anat, sid):
        ...
        return pyr.pmap(r) if len(r) > 0 else None
    @pimms.value
    def subject_boundary_distances(native_paths, _cached_data):
        '''
        ...
        return mapsmerge(
            ...
            pyr.pmap({anat: pimms.lazy_map({sid:curry(f, native_paths, anat, sid)
                                            for sid in HCPLinesDataset.subject_list})
                      for anat in six.iterkeys(native_paths)}))
    @staticmethod
    ...
    def subjects(subject_labels, subject_boundary_distances, clean_retinotopic_maps,
        ...
        def makesub(sid):
            ...
            for h in ('lh','rh'):
                ...
                lbl = sdat.get(h)
                sbd = lookup(subject_boundary_distances, meananat, sid, h)
